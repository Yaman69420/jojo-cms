<x-layout>
    ...
    <div class="mb-10 relative z-10">
        ...
        <h2 class="text-4xl text-yellow-400 bangers transform -skew-x-6 drop-shadow-lg tracking-widest">Edit Part</h2>
    </div>
    ...
    <div class="bg-white p-8 jojo-border jojo-shadow relative z-10">
        <form action="{{ route('admin.parts.update', $part) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-6">
                    <x-form.input name="title" label="Title" :value="$part->title" />
                    <x-form.input name="number" label="Part Number" type="number" :value="$part->number" />
                    <x-form.input name="release_year" label="Release Year" type="number" :value="$part->release_year" />
                    <x-form.textarea name="summary" label="Summary" rows="4" :value="$part->summary" />
                </div>
                <div class="space-y-6">
                    <x-form.input name="trailer_url" label="Trailer URL (YouTube)" :value="$part->trailer_url" />
                    <div class="flex flex-col gap-4">
                        ...
                        @if($part->media->first())
                            <div class="w-48">
                                ...
                                <img src="{{ asset('storage/' . $part->media->first()->path) }}" class="w-full jojo-border">
                            </div>
                        @endif
                    </div>
                </div>
