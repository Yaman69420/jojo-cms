<x-layout>
    <x-slot:title>Parts</x-slot:title>

    <div class="mb-10 flex flex-col sm:flex-row justify-between items-center gap-4 relative z-10">
        <div>
            <h2 class="text-4xl text-yellow-400 bangers transform -skew-x-6 drop-shadow-lg tracking-widest text-shadow-lg">Parts</h2>
            <p class="text-lg font-bold text-fuchsia-300 mt-1 uppercase tracking-wider">A history of the series.</p>
        </div>
    </div>
        ...
        @forelse($parts as $part)
            <div class="bg-white text-purple-900 jojo-border jojo-shadow overflow-hidden group hover:scale-[1.02] transition-transform flex flex-col">
                <div class="h-48 bg-purple-900 relative overflow-hidden flex items-center justify-center border-b-4 border-slate-900">
                    @if($part->poster_path)
                        <img src="{{ asset('storage/' . $part->poster_path) }}" class="object-cover w-full h-full opacity-80 group-hover:opacity-100 transition-opacity">
                ...
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t-2 border-slate-100">
                        <a href="{{ route('parts.show', $part) }}" class="text-indigo-600 font-black uppercase tracking-widest hover:underline">VIEW PART</a>
                    </div>
                </div>
            </div>
        @empty
